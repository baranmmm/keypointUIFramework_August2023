Feature: Work Experience Section
  ...
  Scenario Outline: Candidate completes the Experience section successfully
    ...
    Then verifies the additional experience "fieldsName"
    ...
    Then verifies landing on "Education" page
 ...
 # Scenario: Candidate must choose any option in the Notice Period Dropdown when the dropdown is activated
...
#    When the candidate clicks on the Add More Experience button while the mandatory fields are not filled
...
#
#  Scenario Outline: Additional fields should be oppened through ADD MORE button
